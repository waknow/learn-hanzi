...
describe('syncOnce', () => {
  ...
  it('首次使用且本地有数据、服务端为空 → 推送本地（首次迁移）', async () => {
    ...
    const fetchMock = vi
      ...
      .mockResolvedValueOnce(
        new Response(
          JSON.stringify({ weightData: {}, stats: {}, config: {}, updatedAt: null }),
          { status: 200 }
        )
      )
  ...
  it('服务端有数据 → 拉取覆盖本地', async () => {
    ...
    vi.stubGlobal(
      'fetch',
      vi.fn().mockResolvedValue(new Response(JSON.stringify(serverState), { status: 200 })) as never
    );
  ...
  it('服务端返回非 200 时静默返回', async () => {
    vi.stubGlobal('fetch', vi.fn().mockResolvedValue(new Response('err', { status: 500 })) as never);
    await expect(syncOnce()).resolves.toBeUndefined();
  });
});
