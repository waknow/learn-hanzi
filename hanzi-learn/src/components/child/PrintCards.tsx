        ...
        <div className="max-w-4xl mx-auto flex flex-wrap items-center gap-2">
          ...
          <div className="flex rounded bg-gray-50 overflow-hidden">
            {SIZE_LABELS.map((s) => (
              <button
                key={s.value}
                onClick={() => setConfig({ size: s.value })}
          ...
          <div className="flex rounded bg-gray-50 overflow-hidden">
            {CUT_OPTIONS.map((o) => (
              <button
                key={o.value}
                onClick={() => setConfig({ cutLine: o.value })}
          ...
          <div className="flex rounded bg-gray-50 overflow-hidden">
            {COEFFICIENT_OPTIONS.map((co) => (
              <button
                key={co.value}
                onClick={() => setConfig({ coefficient: co.value })}
          ...
          <button
            onClick={() => setConfig({ showPinyin: !config.showPinyin })}
          ...
          <button
            onClick={() => setConfig({ showColor: !config.showColor })}
          ...
          <span className="text-xs text-gray-300">
            {expandedChars.length} 卡 · {pages.length} 页
          </span>
                ...
                return (
                  ...
                  >
                    {char && (
                      <>
                        <span
                          style={{ fontSize: layout.charSize, lineHeight: 1.2, color: "inherit" }}
                        >
                          {char}
                        </span>
